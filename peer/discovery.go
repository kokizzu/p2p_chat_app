...
func listenForBroadcast(pc net.PacketConn) {
	...
	if val, has := helper.ConnectedHosts[peerAddress]; !has {
		helper.InitConn(peerAddress, ip, messageAttributes[0], messageAttributes[1])
	} else {
		one := make([]byte, 1)
		if _, err := val.Conn.Read(one); err != nil {
			helper.InitConn(peerAddress, ip, messageAttributes[0], messageAttributes[1])
		}
	}
}
